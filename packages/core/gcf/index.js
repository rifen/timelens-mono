...
exports.toGenericGCF = toGenericGCF;
exports.toGraphGCF = toGraphGCF;
exports.toGCF = toGCF;
const gcf_1 = require("@blackwell-systems/gcf");
/**
 * Serializes arbitrary structured data to GCF (generic tabular profile).
 */
function toGenericGCF(data) {
    return (0, gcf_1.encodeGeneric)(data);
}
/**
 * Formats scan results into a GCF graph payload (code graph profile) with symbols and edges.
 */
function toGraphGCF(results, options = {}) {
    ...
    for (const scan of scanArray) {
        ...
        for (let i = 0; i < scan.items.length; i++) {
            ...
            symbols.push({
                ...
                provenance:
                    item.source === "context" ? "context_clues" : "heuristic",
                distance: 1,
...
function toGCF(data, options = {}) {
    ...
    if (Array.isArray(data)) {
        if (data.length > 0 && "items" in data[0]) {
            // Array of ScanResult
            const flattened = data.flatMap((scan) =>
                scan.items.map((item) => ({
            ...
            return (0, gcf_1.encodeGeneric)({ durations: flattened });
        } else {
            ...
            return (0, gcf_1.encodeGeneric)({ durations: items });
        }
    }
    if ("items" in data) {
        const scan = data;
